...
class ACE(nn.Module):
    def __init__(self, config_text, norm_nc, label_nc, ACE_Name=None, status='train', spade_params=None, use_rgb=True):
        ...
        self.Gsas = GSAS(self.style_length)
    ...
    def forward(self, x, segmap, style_codes=None, obj_dic=None):
        ...
        # Part 0. consider hidden representations between superpixels
        style_codes = self.Gsas(style_codes)
                ...
                for i in range(b_size):
                    for j in range(segmap.shape[1]):
                        ...
                        if component_mask_area > 0:
                            ...
                            if self.status == 'ttttttt' and self.save_npy and self.ACE_Name=='up_2_ACE_0':
...
class GSAS(nn.Module):
    def __init__(self, style_length):
        super().__init__()
        self.coefficient = nn.Conv2d(2, 1, kernel_size=1)
        self.softmax = nn.Softmax(dim=1)
        self.fc = nn.Linear(style_length * 2, style_length)

    def forward(self, style_code):
        [b_size, s_size, c_size] = style_code.shape
        r_style_code = style_code.reshape(b_size * s_size, 1, c_size)
        e_style_code = r_style_code.expand(-1, c_size, -1)
        t_style_code = e_style_code.permute(0, 2, 1)

        e_e_style_code = e_style_code.unsqueeze(1)
        t_e_style_code = t_style_code.unsqueeze(1)

        coefficient = torch.cat((e_e_style_code, t_e_style_code), 1)
        coefficient = self.coefficient(coefficient).squeeze()
        weights = self.softmax(coefficient)

        new_style_code = t_style_code * weights
        new_style_code = new_style_code.mean(1)

        #output = torch.cat((style_code.reshape(b_size * s_size, c_size), new_style_code), -1)
        #output = self.fc(output).reshape(b_size, s_size, c_size)
        output = style_code + new_style_code.reshape(b_size, s_size, c_size)
        return output
